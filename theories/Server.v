...
Definition server_client_communication (uname : username) (cc : client_connection) : result unit :=
...
(** Do initial processing of client connection, and then start communicating
    with the client *)
Definition init_client_comms (cc : client_connection) : result unit :=
...
*)
Definition server_accept_thread (socket_fd : file_descr) : result unit :=
    repeat_until_timeout max_int (fun _ =>
        let '(client_descr, client_addr) := accept socket_fd in
        let* _ <= log Log_Debug 
            ("Accepted client socket " ++ (string_of_socket_addr client_addr)) #;
        _ <- create client_connection (result unit) init_client_comms
            {|cc_uname := dummy_username; cc_descr := client_descr; cc_addr := client_addr|} ;;
        return Recurse
    ).

Definition server_control_thread (_ : unit) : result unit :=
    repeat_until_timeout max_int (fun _ => 
...
*)
Definition server (host : string) (portno : port) : result unit :=
    ...
    accept_thread <- create file_descr (result unit) server_accept_thread socket_fd ;;
    control_thread <- create unit (result unit) server_control_thread tt ;;
